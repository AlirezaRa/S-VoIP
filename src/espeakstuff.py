...
def mainSpeechEng():
    ...
    # and hit enter by @stop variable. @decision is required to prevent sox
    # record and save stuff if not wanted.
    ...
    while decision == 'y':
        ...
        if yousaid:
            call(str('PULSE_SINK=null espeak "' + str(yousaid) + '" -s 100'),
                 shell=True)
...
def mainSpeech(lang):
    # For the sake of accuracy, sox records each of your sentence and saves it
    # in temp. speechRecognition then returns a string of the content. That is
    # passed for synthesis to speech by say() for languages other than English.
    # Since sox has to record only one sentence, it needs to be killed
    # at some point. It would be killed if you press 'y' and hit enter by @stop
    # variable. @decision is required to prevent sox record and save stuff if
    # not wanted.
    decision = str(raw_input("Are you sure? (y/n) "))
    while decision == 'y':
        print "**********"
...
def sayAES(ct):
    # Writes every character of the ciphertext in a newline inside a textfile
    # so that espeak reads characters one by one. Also espeak is used with -k 2
    # so that it reads x for x and Capical x for X where x and X are letters of
    # the alphabet.
    with open("../data/temp/ciphertext.txt", 'w') as cipher:
        ct = str(ct)
        for i in range(len(ct)):
            cipher.write(str("%c\n" % (str(ct[i]))))
    p = subprocess.Popen(str("espeak -f ../data/temp/ciphertext.txt -s 50 -k" +
                             " 2 -w ../data/temp/ct.wav"), shell=True)
    p.communicate()
    p = subprocess.Popen(str("PULSE_SINK=null espeak 'Warning, Warning," +
                             "Incoming Ciphertext.' -s 100"), shell=True)
    p.communicate()
    time.sleep(2)
    p = subprocess.Popen(str("PULSE_SINK=null cvlc --play-and-exit" +
                             " ../data/temp/ct.wav"), shell=True)
    p.communicate()
    time.sleep(1)
